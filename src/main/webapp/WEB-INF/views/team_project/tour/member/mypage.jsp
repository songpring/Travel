...
<%-- 			<div style="background-image: url(resources/upload/profile/${mb.profile_photo }); width: 200px; height: 200px; border-radius: 50%;'" id="image_section"></div> --%>
			<img id="image_section" name="" src="<c:url value='/resources/upload/profile/${mb.profile_photo }'/>" alt="profile" style="width:200px; border-radius: 50%;" />
					</c:otherwise>
				</c:choose>	
						...
						<input type="file" id="ex_file" class="imgInput" accept=".jpg, .png, .jpeg" name="file" sizes="600px">
		...
		$(".imgInput").change(function() {
			readURL(this);
				...
				<div class="col-md-9 col-sm-9 animate-box">
					...
					<button type="button" onclick="location.href='reservationDetails'" class="btn btn-default">예매 내역 조회</button>
					...
					<a href='<c:url value="/galleryId?board_id=gallery&regi_id=${regi_id }"/>'>
												<button type="button" class="btn btn-default">My위스타그램</button></a><br>
					<button type="button" onclick="location.href='myinfo'" class="btn btn-default">나의 정보</button>
					<button type="button" onclick="location.href='update'" class="btn btn-default">회원 수정</button>
					<button type="button" onclick="location.href='delete'" class="btn btn-default">회원 탈퇴</button>
					<button id="btn-send" type="button"	onclick="location.href='message'" class="btn btn-default">받은 쪽지함</button><br>
				</div>
