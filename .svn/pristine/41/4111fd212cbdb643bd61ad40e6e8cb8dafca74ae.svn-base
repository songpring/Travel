...
function delete1(){
   ...
   if(result == true) {
      location.href = '<c:url value="/together/delete?seq=${Article.seq }"/>';
      } 
}
            ...
            <div class="row">
               <div>
                  <table class="table table-hover">
                     <tr>
                        <td>작성자 : <a href='<c:url value="/sendMessage?nickname=${Article.nickName }"/>'>${Article.nickName }</a></td>
                        
                        
                        
                        
                        <td>작성일 : ${Article.regi_date }</td>
                        <td>조회수 : ${Article.readcount }</td>
                     </tr>
                     <tr>
                        <td>${Article.board_id } 게시판</td>
                        <td colspan="2">제목 : ${Article.subject }</td>
                     </tr>
                     <tr>
                        <td>여행지 </td>
                        <td colspan="2">${Article.country }</td>
                     </tr>
                     <tr>
                        <td colspan="3" style="height: 400px;">${Article.content }</td>
                     </tr>
                  <tr>
<%--                   <td><a href='<c:url value="together "/>'>목록으로</a></td> --%>
                  <td>
                     <c:if test='${sessionScope.id == Article.regi_id}'>
                     <input type="button" value="글수정" onclick="location.href='<c:url value="/togetherUpdate?seq=${Article.seq }"/>'" >
                     ...
                     <tr>
                        <c:choose>
                           <c:when test='${sessionScope.id != null}'>
                              <form action='<c:url value="togetherComment "/>' method="post" name="fr">
