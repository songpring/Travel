				...
				<div class="row">
					<div class="about-flex" id="airport">
						...
						<div class="col-three-forth animate-box">
							<h2>문화</h2><p>(총 ${pageBean.count } 개)</p>
							<div class="row">
								<div class="col-md-12">
								...
								<div class="col-md-12 text-center">
										...
										<ul class="pagination">
											<c:if test="${pageBean.startPage > pageBean.pageBlock }">
												<li><a href='<c:url value="/culture?pageNum=${pageBean.startPage-pageBean.pageBlock }&searchSelect=${searchSelect }&search=${search }"/>'>[이전]</a></li>
											</c:if>
											
											<c:forEach var="i" begin="${pageBean.startPage }" end="${pageBean.endPage }" step="1">
												<li><a href='<c:url value="/culture?pageNum=${i }&searchSelect=${searchSelect }&search=${search }"/>'>${i }</a></li>
											</c:forEach>
											
											<c:if test="${pageBean.endPage < pageBean.pageCount}">
												<li><a href='<c:url value="/culture?pageNum=${pageBean.startPage+pageBean.pageBlock }&searchSelect=${searchSelect }&search=${search }"/>'>[다음]</a></li>
											</c:if>
											</ul>
										</div>	
										</div>
										...
										<!--검색기능-->
										<form action='<c:url value="/culture"/>' method="get" style="float: right;  width: 440px;">
										<div class="col-md-3">
