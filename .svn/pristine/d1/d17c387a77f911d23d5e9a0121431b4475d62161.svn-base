				...
				<div>
					<table class="table table-hover">
						...
						<tr>
							...
							<td colspan="3">제목 : ${Article.subject }</td>
						</tr>
						<tr>
							<td colspan="4">본문 : <br>${Article.content }</td>
						</tr>
						<tr>
<%-- 							<td><a href='<c:url value="freeboard "/>'>목록으로</a></td> --%>
							<td>
							<c:if test='${sessionScope.id == Article.regi_id}'>
								<input type="button" value="글수정" onclick="location.href='<c:url value="/freeboardUpdate?seq=${Article.seq }"/>'" >
								...
								function delete1(){
									...
									if(result == true) {
										location.href = '<c:url value="/freeboard/delete?seq=${Article.seq }"/>';
										} 
								}
								function back(){
									history.back();
							...
							<tr>
								...
								<c:choose>
									<c:when test='${sessionScope.id != null}'>
										<form action='<c:url value="freeboardComment "/>' method="post" name="fr">
